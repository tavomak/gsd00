...
## API Routes
`/api/register` (form+reCAPTCHA)

## SEO routes (dynamic, per-host)
`/sitemap.xml` → `pages/sitemap.xml.js` — static routes from `siteConfig.navKeys` + Hygraph projects filtered by `siteConfig.category`; emits hreflang alternates
`/robots.txt` → `pages/robots.txt.js` — points at `https://{siteConfig.domain}/sitemap.xml`

Both detect the active site via `siteFromHost(req.headers.host)` from `@/config/sites`.

## SEO helpers
`utils/seo.js` (re-exported from `@/utils`):
- `buildOrganizationSchema(config)` → JSON-LD Organization (name, url, logo, sameAs[instagram])
- `buildWebSiteSchema(config)` → JSON-LD WebSite

Templates build the schema from `useSite().config` and pass via `Layout`'s `schema` prop.
