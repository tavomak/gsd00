...
const ProjectCard = ({ project, lang }) => (
  ...
  >
    ...
    >
      <h3 className="text-base text-white uppercase">{project?.title}</h3>
      {project?.description && (
        <p className="mt-1 text-base text-neutral-300 line-clamp-2">
          {project.description}
        </p>
      )}
      {project?.category && (
    ...
    <div className="p-4 border-t md:hidden border-neutral-800">
      <h3 className="text-base font-bold transition-colors duration-300 group-hover:text-primary-color uppercase">
        {project.title}
      </h3>
    </div>
  </Link>
