            ...
            <AnimatePresence mode="wait" initial={false}>
              {hovered && item.href ? (
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <span className="text-base font-bold tracking-widest uppercase text-primary-color">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  <h3 className="mt-1 text-base text-white uppercase">
                    {item.title}
                  </h3>
                </motion.div>
              )}
...
const StickyTwoColumn = ({ items = [] }) => {
  ...
  const handleInView = useCallback((item, index) => {
    setActiveItem(item);
          ...
          <div className="relative flex-1 w-full min-h-0">
            ...
            <div className="flex gap-8">
              <Button
                ...
                onClick={() => window.open(activeItem.href, '_blank')}
              >
                {t('go_to_project')}
              </Button>
              <Button
                className="btn btn-primary mt-6"
                onClick={() => window.open('https://83s.cl', '_blank')}
              >
                {t('view_all_projects')}
              </Button>
            </div>
